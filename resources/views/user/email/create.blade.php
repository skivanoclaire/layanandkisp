...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('user.email.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm mb-1">Username yang diajukan (@kaltaraprov.go.id)</label>
                    <input type="text" name="username" value="{{ old('username') }}" required
                        class="w-full border rounded p-2" placeholder="misal: budi.susanto">
                    @error('username')
                        <p class="text-red-600 text-sm">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm mb-1">Password - Minimal 8 Karakter, Kombinasi huruf, angka dan simbol
