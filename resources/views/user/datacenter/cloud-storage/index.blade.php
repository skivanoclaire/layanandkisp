...
@section('title', '- Permohonan Cloud Storage')
@section('header-title', 'Cloud Storage')

@section('content')
<div class="container mx-auto px-4">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-purple-700">Riwayat Permohonan Cloud Storage</h1>
        <a href="{{ route('user.datacenter.cloud-storage.create') }}"
    ...
    <div class="mb-6 bg-white rounded-lg shadow-md p-4">
        <form method="GET" action="{{ route('user.datacenter.cloud-storage.index') }}" class="flex gap-4 items-end">
            <div class="flex-1">
            ...
            <a href="{{ route('user.datacenter.cloud-storage.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded font-semibold">
                Reset
            </a>
        </form>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-purple-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Kapasitas & Tipe</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Akses URL</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">
                        <span class="font-semibold">{{ $item->kapasitas_gb }} GB</span><br>
                        <span class="text-xs text-gray-500">{{ $item->tipe }}</span>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-900">
                        @if($item->akses_url)
                            <a href="{{ $item->akses_url }}" target="_blank" class="text-purple-600 hover:text-purple-800 underline">
                                Link Akses →
                            </a>
                        @else
                            -
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="{{ route('user.datacenter.cloud-storage.show', $item) }}"
                           class="text-purple-600 hover:text-purple-800 font-semibold">
                            Detail →
                        </a>
                        @if($item->status === 'selesai')
                        <a href="{{ route('user.datacenter.cloud-storage.survey', $item->id) }}"
                ...
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                        Belum ada permohonan cloud storage.
                    </td>
                </tr>
                @endforelse
            </tbody>
