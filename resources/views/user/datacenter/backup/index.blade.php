...
@section('title', '- Permohonan Backup')
@section('header-title', 'Backup')

@section('content')
<div class="container mx-auto px-4">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-purple-700">Riwayat Permohonan Backup</h1>
        <a href="{{ route('user.datacenter.backup.create') }}"
    ...
    <div class="mb-6 bg-white rounded-lg shadow-md p-4">
        <form method="GET" action="{{ route('user.datacenter.backup.index') }}" class="flex gap-4 items-end">
            <div class="flex-1">
            ...
            <a href="{{ route('user.datacenter.backup.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded font-semibold">
                Reset
            </a>
        </form>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-purple-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Tipe Backup</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Jadwal & Retensi</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">
                        @php
                            $types = [];
                            if($item->backup_virtual_machine) $types[] = 'VM';
                            if($item->backup_aplikasi) $types[] = 'Aplikasi';
                            if($item->backup_database) $types[] = 'Database';
                        @endphp
                        {{ implode(', ', $types) }}
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-900">
                        {{ $item->jadwal_backup }}<br>
                        <span class="text-xs text-gray-500">Retensi: {{ $item->retensi_hari }} hari</span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="{{ route('user.datacenter.backup.show', $item) }}"
                           class="text-purple-600 hover:text-purple-800 font-semibold">
                            Detail →
                        </a>
                        @if($item->status === 'selesai')
                        <a href="{{ route('user.datacenter.backup.survey', $item->id) }}"
