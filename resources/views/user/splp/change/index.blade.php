...
<div class="container mx-auto p-6">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Perubahan / Perpanjangan</h1>
            <p class="text-gray-600 mt-1">Permohonan perubahan/perpanjangan layanan SPLP Anda.</p>
        </div>
        <a href="{{ route('user.splp.change.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-semibold">+ Ajukan Baru</a>
    </div>
            ...
            <tbody class="divide-y">
                @forelse ($items as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ ucfirst($item->kategori) }} ({{ $item->jenis_perubahan }})</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($item->isEditableByOwner())
                                <a href="{{ route('user.splp.change.edit', $item->id) }}" class="text-blue-600 hover:underline">Edit</a>
                                <form action="{{ route('user.splp.change.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus permohonan ini?')">
                                    @csrf @method('DELETE')
