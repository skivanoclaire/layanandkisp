    ...
    <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
        ...
        <div class="border-b pb-4">
            <div class="flex justify-between items-start">
                <div>
                    <h2 class="text-xl font-semibold text-gray-800">{{ $tteCertificateUpdate->ticket_no }}</h2>
                    <p class="text-sm text-gray-600 mt-1">Diajukan pada {{ $tteCertificateUpdate->submitted_at->format('d M Y H:i') }}</p>
                </div>
                <span class="inline-block px-4 py-2 rounded-full {{ $tteCertificateUpdate->getStatusBadgeClass() }} font-semibold">
        ...
        <div class="border-t pt-4">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <p class="text-gray-700">{{ $tteCertificateUpdate->keterangan_admin }}</p>
            </div>
        </div>
        @endif
        ...
        <!-- Back Button -->
        <div class="border-t pt-4">
            <a href="{{ route('user.tte.certificate-update.index') }}" class="inline-block bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg font-semibold transition">
                Kembali
            </a>
        </div>
    </div>
