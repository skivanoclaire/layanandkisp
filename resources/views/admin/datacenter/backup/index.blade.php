...
@section('title', '- Kelola Backup')
@section('header-title', 'Kelola Backup')

@section('content')
<div class="container mx-auto px-4">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-purple-700">Kelola Permohonan Backup</h1>
    </div>
    ...
    <div class="mb-6 bg-white rounded-lg shadow-md p-4">
        <form method="GET" action="{{ route('admin.datacenter.backup.index') }}" class="flex gap-4 items-end">
            <div class="flex-1">
            ...
            <a href="{{ route('admin.datacenter.backup.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded font-semibold">
                Reset
            </a>
        </form>
    </div>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">
                        @php
                            ...
                            if($item->backup_aplikasi) $types[] = 'App';
                            if($item->backup_database) $types[] = 'DB';
                        @endphp
                        {{ implode(', ', $types) }}
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-900">
                        {{ $item->jadwal_backup }}<br>
                        <span class="text-xs text-gray-500">{{ $item->retensi_hari }}d</span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <a href="{{ route('admin.datacenter.backup.show', $item) }}"
