...
<div class="container mx-auto px-4 max-w-6xl">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="border-b pb-4 mb-6">
                    <div class="flex justify-between items-start">
                        <div>
                            <h2 class="text-2xl font-semibold text-gray-800">{{ $ttePassphraseReset->ticket_no }}</h2>
                            <p class="text-sm text-gray-600 mt-1">Diajukan pada {{ $ttePassphraseReset->submitted_at->format('d M Y H:i') }}</p>
                        </div>
                        <span class="inline-block px-4 py-2 rounded-full {{ $ttePassphraseReset->getStatusBadgeClass() }} font-semibold text-sm">
                ...
                <div class="border-t mt-6 pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-gray-900">{{ $ttePassphraseReset->user->username ?? '-' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 mb-1">Email User</label>
                            <p class="text-gray-900">{{ $ttePassphraseReset->user->email ?? '-' }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                @if($ttePassphraseReset->processedBy)
                                <p class="text-sm text-gray-500">oleh {{ $ttePassphraseReset->processedBy->name }}</p>
                                @endif
                            </div>
                        </div>
                        @endif
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-lg shadow-md p-6 sticky top-4">
                ...
                <form action="{{ route('admin.tte.passphrase-reset.update-status', $ttePassphraseReset) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <select name="instansi" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-purple-500">
                            ...
                            @foreach($unitKerjas as $uk)
                                <option value="{{ $uk->nama }}" {{ old('instansi', $ttePassphraseReset->instansi) == $uk->nama ? 'selected' : '' }}>
                                    {{ $uk->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="jabatan" value="{{ old('jabatan', $ttePassphraseReset->jabatan) }}"
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-purple-500" required>
                            <option value="menunggu" {{ $ttePassphraseReset->status == 'menunggu' ? 'selected' : '' }}>Menunggu</option>
                            <option value="diproses" {{ $ttePassphraseReset->status == 'diproses' ? 'selected' : '' }}>Diproses</option>
                            <option value="selesai" {{ $ttePassphraseReset->status == 'selesai' ? 'selected' : '' }}>Selesai</option>
                            <option value="ditolak" {{ $ttePassphraseReset->status == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="keterangan_admin" rows="4" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-purple-500"
                            placeholder="Keterangan untuk pemohon (opsional)">{{ old('keterangan_admin', $ttePassphraseReset->keterangan_admin) }}</textarea>
                    </div>
                ...
                <div class="mt-6 pt-6 border-t">
                    <a href="{{ route('admin.tte.passphrase-reset.index') }}" class="block w-full text-center bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg font-semibold transition">
                        Kembali
                    </a>
                </div>
            </div>
    ...
    <div class="mt-6 bg-white rounded-lg shadow-md p-6">
        ...
        <div class="bg-gray-50 rounded-lg p-4">
            <p class="text-gray-700 whitespace-pre-wrap">{{ $ttePassphraseReset->keterangan_admin }}</p>
        </div>
    </div>
    @endif
</div>
