    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('admin.rekomendasi.verifikasi.export-pdf', $proposal->id) }}"
                    target="_blank"
                ...
                @if($proposal->status === 'diajukan')
                    @if($proposal->verifikasi)
                        @if(in_array($proposal->verifikasi->status, ['menunggu', 'sedang_diverifikasi']))
                        ...
                        @endif
                    @else
                        <!-- Fallback: Show button to start verification if no verifikasi record exists -->
        ...
        <!-- Notifikasi Revisi (Perlu Revisi) -->
            ...
            <div class="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
                <div class="flex items-start justify-between flex-wrap gap-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-semibold text-orange-800">Usulan Berstatus Perlu Revisi</h3>
                            <p class="text-sm text-orange-700 mt-1">
                                Kirim pengingat WhatsApp ke pemohon agar segera memperbarui usulannya.
                            </p>
                            <p class="text-xs text-orange-600 mt-1">
                                Nomor tujuan:
                                <span class="font-medium">{{ $proposal->user?->phone ?: 'Tidak tersedia' }}</span>
                            </p>
                        </div>
                    </div>
                    @if($proposal->user?->phone)
                        <form method="POST" action="{{ route('admin.rekomendasi.verifikasi.notify-revisi', $proposal->id) }}"
                            ...
                            @csrf
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition shadow-sm whitespace-nowrap">
        ...
        <!-- Status Persetujuan Kementerian (Admin) -->
        @if($proposal->status === 'disetujui')
            @php
                $statusKementerian = $proposal->statusKementerian;
                $currentKementerianStatus = $statusKementerian?->status ?? 'belum_diproses';
                $statusKementerianColors = [
                    'belum_diproses' => 'bg-gray-50 border-gray-200',
                ...
                $statusKementerianLabels = [
                    'belum_diproses' => 'Belum Diproses ke Kementerian',
            ...
            @endphp
            <div class="rounded-lg border p-6 mb-6 {{ $statusKementerianColors[$currentKementerianStatus] ?? 'bg-gray-50 border-gray-200' }}">
                <div class="flex items-start justify-between">
                    <div class="flex items-start">
                        <div class="flex-shrink-0">
                            <div class="w-12 h-12 rounded-full flex items-center justify-center {{ $currentKementerianStatus === 'disetujui' ? 'bg-green-100' : ($currentKementerianStatus === 'ditolak' ? 'bg-red-100' : 'bg-blue-100') }}">
                                <svg class="h-6 w-6 {{ $currentKementerianStatus === 'disetujui' ? 'text-green-600' : ($currentKementerianStatus === 'ditolak' ? 'text-red-600' : 'text-blue-600') }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="ml-4">
                            ...
                            <span class="inline-flex items-center mt-2 px-3 py-1 rounded-full text-sm font-medium {{ $statusKementerianBadgeColors[$currentKementerianStatus] ?? 'bg-gray-100 text-gray-800' }}">
                                {{ $statusKementerianLabels[$currentKementerianStatus] ?? 'Belum Diproses' }}
                            </span>
                            ...
                            @if($statusKementerian)
                                ...
                                @if($statusKementerian->file_respons_path)
                                    <div class="mt-3">
                                        <a href="{{ asset('storage/' . $statusKementerian->file_respons_path) }}" target="_blank"
                                            class="inline-flex items-center text-blue-600 hover:text-blue-800 text-sm font-medium">
                                            <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                            </svg>
                                            Download Surat Kementerian
                                        </a>
                                    </div>
                                @endif
                            @endif
                    ...
                    <div>
                        <button onclick="showMinistryStatusModal()" class="inline-flex items-center px-4 py-2 bg-yellow-600 text-white text-sm font-medium rounded-lg hover:bg-yellow-700 transition shadow-sm">
                ...
                <div class="flex items-start justify-between flex-wrap gap-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-sm text-gray-500 mt-1">Upload dokumen berita acara (PDF/DOCX). Dokumen ini dapat diunduh oleh admin maupun pemohon.</p>
                            @if($beritaAcara)
                                <div class="mt-3">
                                    <a href="{{ route('admin.rekomendasi.verifikasi.berita-acara.download', $proposal->id) }}"
                                        class="inline-flex items-center text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                                        <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                        </svg>
                                        Download Berita Acara ({{ strtoupper(pathinfo($beritaAcara, PATHINFO_EXTENSION)) }})
                                    </a>
                                    <p class="text-xs text-gray-400 mt-1">{{ basename($beritaAcara) }}</p>
                                </div>
                            @else
                                <p class="mt-3 text-sm text-gray-400 italic">Belum ada berita acara yang diupload.</p>
                            @endif
                        </div>
                    </div>
                    <form method="POST" action="{{ route('admin.rekomendasi.verifikasi.berita-acara.upload', $proposal->id) }}" enctype="multipart/form-data" class="flex items-center gap-2">
                        @csrf
                        <input type="file" name="file_berita_acara" accept=".pdf,.doc,.docx" required
                            class="text-sm text-gray-700 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition shadow-sm whitespace-nowrap">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                            </svg>
                            {{ $beritaAcara ? 'Ganti Berita Acara' : 'Upload Berita Acara' }}
                        </button>
                    </form>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @if($proposal->integrasi_sistem_lain === 'ya' && $proposal->detail_integrasi)
                        <div class="col-span-2">
                            ...
                            <div class="text-gray-900 prose max-w-none">{!! $proposal->detail_integrasi !!}</div>
                        </div>
                    @endif
                    ...
                    @if($proposal->kebutuhan_khusus)
                        <div class="col-span-2">
                            ...
                            <div class="text-gray-900 prose max-w-none">{!! $proposal->kebutuhan_khusus !!}</div>
                        </div>
                    @endif
                    ...
                    @if($proposal->dampak_tidak_dibangun)
                        <div class="col-span-2">
                            ...
                            <div class="text-gray-900 prose max-w-none">{!! $proposal->dampak_tidak_dibangun !!}</div>
                        </div>
                    @endif
                </div>
            </div>
                ...
                @if($proposal->verifikasi)
                    <div class="space-y-6">
                        ...
                        @if($proposal->verifikasi->file_kajian)
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800 mb-3">File Kajian</h3>
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <a href="{{ route('admin.rekomendasi.verifikasi.kajian.download', $proposal->id) }}"
                                       class="inline-flex items-center px-4 py-2 bg-orange-600 text-white text-sm font-medium rounded-lg hover:bg-orange-700">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                        </svg>
                                        Download File Kajian (PDF)
                                    </a>
                                </div>
                            </div>
                        @endif
...
<div id="ministryStatusModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form method="POST" action="{{ route('admin.rekomendasi.verifikasi.ministry-status', $proposal->id) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="menunggu" {{ ($proposal->statusKementerian?->status ?? '') === 'menunggu' ? 'selected' : '' }}>Menunggu Respons</option>
                            <option value="diproses" {{ ($proposal->statusKementerian?->status ?? '') === 'diproses' ? 'selected' : '' }}>Sedang Diproses</option>
                            <option value="disetujui" {{ ($proposal->statusKementerian?->status ?? '') === 'disetujui' ? 'selected' : '' }}>Disetujui</option>
                            <option value="ditolak" {{ ($proposal->statusKementerian?->status ?? '') === 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                            <option value="revisi_diminta" {{ ($proposal->statusKementerian?->status ?? '') === 'revisi_diminta' ? 'selected' : '' }}>Perlu Revisi</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Nomor Surat Respons Kementerian
                        </label>
                        <input type="text" name="nomor_surat_respons" value="{{ old('nomor_surat_respons', $proposal->statusKementerian?->nomor_surat_respons) }}"
                            placeholder="Contoh: B-123/KOMDIGI/2026"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Tanggal Surat Respons
                        </label>
                        <input type="date" name="tanggal_surat_respons" value="{{ old('tanggal_surat_respons', $proposal->statusKementerian?->tanggal_surat_respons?->format('Y-m-d')) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Upload Surat Persetujuan/Respons Kementerian (PDF)
                        </label>
                        <input type="file" name="file_respons" accept=".pdf"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500">
                        @if($proposal->statusKementerian?->file_respons_path)
                            <p class="text-xs text-green-600 mt-1">File sudah ada: {{ basename($proposal->statusKementerian->file_respons_path) }}</p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="catatan" rows="3"
                            placeholder="Catatan tambahan dari Kementerian (opsional)"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500">{{ old('catatan', $proposal->statusKementerian?->catatan_internal) }}</textarea>
                    </div>
                </div>
