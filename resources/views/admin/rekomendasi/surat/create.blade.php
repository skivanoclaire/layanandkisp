...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Buat Surat Rekomendasi</h1>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                ...
                <div>
                    ...
                    <span class="text-blue-900 font-medium ml-2">{{ $proposal->user->name }}</span>
                </div>
                <div>
                    <span class="text-blue-600">Unit Kerja:</span>
                    <span class="text-blue-900 font-medium ml-2">{{ $proposal->pemilikProsesBisnis?->nama ?? '-' }}</span>
                </div>
                <div>
                    <span class="text-blue-600">Tanggal Disetujui:</span>
                    <span class="text-blue-900 font-medium ml-2">{{ $proposal->verifikasi->tanggal_verifikasi->format('d M Y') }}</span>
                </div>
            </div>
        </div>

        <!-- Letter Form -->
        <form method="POST" action="{{ route('admin.rekomendasi.surat.store', $proposal->id) }}">
            @csrf
            ...
            <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="nomor_surat_draft" name="nomor_surat_draft"
                        value="{{ old('nomor_surat_draft', $draftNomor) }}"
                ...
                <div>
                    ...
                    <input type="text" id="perihal" name="perihal"
                        value="{{ old('perihal', 'Rekomendasi Pengembangan Aplikasi ' . $proposal->nama_aplikasi) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        required>
                    @error('perihal')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" id="tujuan_surat" name="tujuan_surat"
                        value="{{ old('tujuan_surat', 'Kementerian Komunikasi dan Informatika RI') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        required>
                    @error('tujuan_surat')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div id="referensi-container" class="space-y-2">
                        @if(old('referensi_hukum'))
                            @foreach(old('referensi_hukum') as $index => $ref)
                ...
                <div>
                    ...
                    <textarea id="isi_surat" name="isi_surat" rows="20"
                        ...
                        required>{{ old('isi_surat', $defaultContent ?? '') }}</textarea>
