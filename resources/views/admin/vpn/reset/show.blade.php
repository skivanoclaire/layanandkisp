...
<div class="container mx-auto px-4 max-w-5xl">
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @if($vpnReset->is_kabupaten_kota)
                        <p class="text-gray-800">{{ $vpnReset->unit_kerja_manual ?? '-' }}</p>
                        <p class="text-xs mt-1">
                            <span class="px-2 py-0.5 bg-purple-100 text-purple-800 rounded font-semibold">Kab/Kota: {{ $vpnReset->kabupaten_kota ?? '-' }}</span>
                        </p>
                    @else
    ...
    <div class="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-6 mb-6">
        ...
        <!-- Form Proses -->
        <form id="formProses" action="{{ route('admin.vpn.reset.process', $vpnReset->id) }}" method="POST" style="display:none;" class="mt-4">
            @csrf
        ...
        <!-- Form Tolak -->
        <form id="formTolak" action="{{ route('admin.vpn.reset.reject', $vpnReset->id) }}" method="POST" style="display:none;" class="mt-4">
            @csrf
    ...
    <div class="bg-blue-50 border-l-4 border-blue-500 rounded-lg p-6 mb-6">
        ...
        <form action="{{ route('admin.vpn.reset.complete', $vpnReset->id) }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-semibold text-blue-700 mb-1">Username VPN Baru: <span class="text-red-500">*</span></label>
                    <input type="text" name="username_vpn_baru" required
                           class="w-full px-3 py-2 border border-blue-300 rounded-lg"
                           placeholder="Username VPN baru yang diberikan">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-blue-700 mb-1">Password VPN Baru: <span class="text-red-500">*</span></label>
                    <input type="text" name="password_vpn_baru" required
                           class="w-full px-3 py-2 border border-blue-300 rounded-lg"
                           placeholder="Password VPN baru yang diberikan">
                </div>
            </div>
            <div class="mb-4">
                ...
                <textarea name="keterangan_admin" rows="3"
                          class="w-full px-3 py-2 border border-blue-300 rounded-lg"
                          placeholder="Informasi tambahan untuk user (opsional)">{{ $vpnReset->keterangan_admin }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-blue-700 mb-1">Catatan Internal (opsional):</label>
                <textarea name="admin_notes" rows="2"
                          class="w-full px-3 py-2 border border-blue-300 rounded-lg"
                          placeholder="Catatan untuk admin">{{ $vpnReset->admin_notes }}</textarea>
            </div>
            <button type="submit" class="mt-3 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded font-semibold">
                Selesaikan & Berikan Kredensial Baru
            </button>
        </form>
    </div>
    @endif

    @if(in_array($vpnReset->status, ['proses', 'selesai']))
    <div class="bg-white rounded-lg shadow-md p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Catatan Admin</h3>
        <form action="{{ route('admin.vpn.reset.update-notes', $vpnReset->id) }}" method="POST">
            @csrf
            <textarea name="admin_notes" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="Tambahkan catatan...">{{ $vpnReset->admin_notes }}</textarea>
            <div class="mt-3">
        ...
        @if($vpnReset->processedBy)
        <div class="mt-4 pt-4 border-t">
            <p class="text-sm text-gray-600">Diproses oleh: <span class="font-semibold">{{ $vpnReset->processedBy->name }}</span></p>
        </div>
        @endif
    </div>
    @endif
