...
<head>
    <meta charset="utf-8">
    <title>Data Permohonan Video Conference</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>Data Permohonan Video Conference</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ Str::limit($request->event_title, 40) }}</td>
                <td>
                    @if($request->event_date_start && $request->event_date_end)
                        @if(\Carbon\Carbon::parse($request->event_date_start)->isSameDay(\Carbon\Carbon::parse($request->event_date_end)))
                            {{ \Carbon\Carbon::parse($request->event_date_start)->format('d/m/Y') }}
                        @else
                            {{ \Carbon\Carbon::parse($request->event_date_start)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($request->event_date_end)->format('d/m/Y') }}
                        @endif
                    @else
                        -
                    @endif
