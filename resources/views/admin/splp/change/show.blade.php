...
@section('title', '- Detail Perubahan/Perpanjangan SPLP')
@section('header-title', 'Detail Permohonan — Perubahan/Perpanjangan (SPLP)')

@section('content')
<div class="container mx-auto p-6">
    <a href="{{ route('admin.splp.change.index') }}" class="text-blue-600 hover:underline text-sm">&larr; Kembali ke daftar</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg shadow-sm border p-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h2 class="text-xl font-bold text-gray-800">{{ ucfirst($item->kategori) }} — {{ $item->service->nama_layanan ?? '-' }}</h2>
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    ...
                    <div><dt class="text-gray-500">Layanan</dt><dd class="font-medium">{{ $item->service->kode_layanan ?? '-' }}</dd></div>
                    <div><dt class="text-gray-500">Jenis Perubahan</dt><dd class="font-medium">{{ ucfirst($item->jenis_perubahan) }}</dd></div>
                    @if ($item->kategori === 'perpanjangan')
                        <div><dt class="text-gray-500">Perpanjangan Sampai</dt><dd class="font-medium">{{ optional($item->perpanjangan_sampai)->format('d/m/Y') ?? '-' }}</dd></div>
                    @endif
                    <div class="md:col-span-2"><dt class="text-gray-500">Detail Perubahan</dt><dd class="whitespace-pre-line">{{ $item->detail_perubahan }}</dd></div>
                    @if ($item->analisis_dampak)
                        <div class="md:col-span-2"><dt class="text-gray-500">Analisis Dampak</dt><dd class="whitespace-pre-line">{{ $item->analisis_dampak }}</dd></div>
                    @endif
                </dl>
                @if ($item->surat_path)
                    <div class="mt-4 text-sm"><a href="{{ Storage::url($item->surat_path) }}" target="_blank" class="text-blue-600 hover:underline">📎 Surat Permohonan</a></div>
                @endif
            </div>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-lg shadow-sm border p-6">
                ...
                <form action="{{ route('admin.splp.change.status', $item->id) }}" method="POST" class="space-y-4" x-data="{ status: '{{ $item->status }}' }">
                    @csrf
                    <div>
                        ...
                        <select name="status" x-model="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            @foreach (\App\Models\SplpChangeRequest::statusLabels() as $val => $label)
                                @continue($val === 'draft')
                                <option value="{{ $val }}" @selected($item->status === $val)>{{ $label }}</option>
                            @endforeach
                        </select>
                        <p class="text-xs text-gray-500 mt-1">Perubahan minor dapat tanpa keputusan; signifikan perlu keputusan.</p>
                    </div>
                    <div class="border rounded-lg p-3 bg-gray-50 space-y-2">
                        <p class="text-xs font-semibold text-gray-600">Checklist</p>
                        @foreach (['check_administrasi' => 'Administrasi & endpoint teridentifikasi', 'check_dampak' => 'Analisis dampak selesai'] as $field => $label)
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Analisis Dampak</label>
                        <textarea name="analisis_dampak" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">{{ $item->analisis_dampak }}</textarea>
                    </div>
                    <div>
                    ...
                    <p class="text-xs text-gray-500">Saat <strong>Selesai</strong>, perubahan dicatat ke audit trail layanan. Edit konfigurasi via Master Data → Layanan.</p>
                </form>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border p-6 text-sm">
                ...
                <dl class="space-y-2">
                    ...
                    <div class="flex justify-between"><dt class="text-gray-500">Analisis Dampak</dt><dd>{{ optional($item->verif_teknis_at)->format('d/m/Y H:i') ?? '-' }}</dd></div>
