...
@section('title', '- Kelola Perubahan/Perpanjangan SPLP')
@section('header-title', 'Kelola Permohonan — Perubahan/Perpanjangan (SPLP)')

@section('content')
<div class="container mx-auto p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-1">Perubahan / Perpanjangan (V4)</h1>
    <p class="text-gray-600 mb-6">Nilai dampak & terapkan perubahan konfigurasi endpoint.</p>
    ...
    <form method="GET" class="bg-white rounded-lg shadow-sm border p-4 mb-4 flex flex-wrap items-end gap-3">
        ...
        <a href="{{ route('admin.splp.change.index') }}" class="px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-100">Reset</a>
    </form>
            ...
            <tbody class="divide-y">
                @forelse ($items as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs font-semibold @class(['bg-gray-100 text-gray-700' => $item->jenis_perubahan === 'minor', 'bg-amber-100 text-amber-800' => $item->jenis_perubahan === 'signifikan'])">{{ ucfirst($item->jenis_perubahan) }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center"><a href="{{ route('admin.splp.change.show', $item->id) }}" class="text-blue-600 hover:underline">Detail</a></td>
                    </tr>
                @empty
