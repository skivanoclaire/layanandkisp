...
@section('title', '- Detail Penonaktifan/Pencabutan SPLP')
@section('header-title', 'Detail Permohonan — Penonaktifan/Pencabutan (SPLP)')

@section('content')
<div class="container mx-auto p-6">
    <a href="{{ route('admin.splp.deactivation.index') }}" class="text-blue-600 hover:underline text-sm">&larr; Kembali ke daftar</a>
    ...
    @if ($item->is_darurat)
        <div class="bg-red-100 border border-red-300 text-red-800 px-4 py-3 rounded my-4 text-sm font-semibold">⚠ Jalur Darurat (insiden keamanan) — penonaktifan dapat dieksekusi lebih dahulu, administrasi menyusul.</div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg shadow-sm border p-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h2 class="text-xl font-bold text-gray-800">{{ ucfirst($item->jenis_tindakan) }} — {{ $item->target_type === 'service' ? 'Layanan' : 'Konsumen' }}</h2>
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    ...
                    <div><dt class="text-gray-500">Objek Target</dt><dd class="font-medium">
                        @if ($item->target_type === 'service')
                            {{ $item->service->nama_layanan ?? '-' }} ({{ $item->service->kode_layanan ?? '-' }})
                        @else
                            {{ $item->consumer->nama_konsumen ?? '-' }} → {{ $item->consumer->service->nama_layanan ?? '-' }}
                        @endif
                    </dd></div>
                    <div><dt class="text-gray-500">Status Objek Saat Ini</dt><dd class="font-medium">{{ ucfirst($item->target_type === 'service' ? ($item->service->status ?? '-') : ($item->consumer->status ?? '-')) }}</dd></div>
                    <div class="md:col-span-2"><dt class="text-gray-500">Alasan</dt><dd class="whitespace-pre-line">{{ $item->alasan }}</dd></div>
                </dl>
                @if ($item->surat_path)
                    <div class="mt-4 text-sm"><a href="{{ Storage::url($item->surat_path) }}" target="_blank" class="text-blue-600 hover:underline">📎 Surat / Rekomendasi</a></div>
                @endif
            </div>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-lg shadow-sm border p-6">
                ...
                <form action="{{ route('admin.splp.deactivation.status', $item->id) }}" method="POST" class="space-y-4" x-data="{ status: '{{ $item->status }}' }">
                    @csrf
                    <div>
                    ...
                    <div class="border rounded-lg p-3 bg-gray-50 space-y-2">
                        ...
                        @foreach (['check_administrasi' => 'Administrasi & objek teridentifikasi', 'check_dampak' => 'Dampak ke konsumen lain dinilai'] as $field => $label)
                    ...
                    <p class="text-xs text-gray-500">Saat <strong>Selesai</strong>, objek di-{{ $item->jenis_tindakan }} ({{ $item->jenis_tindakan === 'cabut' ? 'dicabut' : 'nonaktif' }}) + diarsipkan ke audit trail.</p>
                </form>
            </div>
