...
@section('title', '- Detail Endpoint Penyedia SPLP')
@section('header-title', 'Detail Permohonan — Endpoint Penyedia (SPLP)')

@section('content')
<div class="container mx-auto p-6">
    <a href="{{ route('admin.splp.provider.index') }}" class="text-blue-600 hover:underline text-sm">&larr; Kembali ke daftar</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg shadow-sm border p-6">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    ...
                    <div><dt class="text-gray-500">No. HP</dt><dd class="font-medium">{{ $item->no_hp }}</dd></div>
                    <div class="md:col-span-2"><dt class="text-gray-500">Backend URL</dt><dd class="font-mono break-all">{{ $item->backend_url }}</dd></div>
                    <div><dt class="text-gray-500">Route Path</dt><dd class="font-mono">{{ $item->route_path ?? '-' }}</dd></div>
                    <div><dt class="text-gray-500">Autentikasi</dt><dd class="font-medium">{{ strtoupper($item->auth_type) }}</dd></div>
                    <div><dt class="text-gray-500">Klasifikasi Data</dt><dd class="font-medium">{{ ucfirst($item->klasifikasi_data) }}</dd></div>
                    <div><dt class="text-gray-500">CIA</dt><dd class="font-medium">{{ $item->dc_confidentiality ?? '-' }} / {{ $item->dc_integrity ?? '-' }} / {{ $item->dc_availability ?? '-' }}</dd></div>
                    <div class="md:col-span-2"><dt class="text-gray-500">Deskripsi</dt><dd>{{ $item->deskripsi ?? '-' }}</dd></div>
                </dl>
                ...
                <div class="mt-4 flex flex-wrap gap-3 text-sm">
                    ...
                    @endif
                    @if ($item->openapi_doc_path)
                        <a href="{{ Storage::url($item->openapi_doc_path) }}" target="_blank" class="text-blue-600 hover:underline">📎 Dokumentasi API</a>
                    @endif
                </div>

                @if ($item->splp_service_id)
                    <div class="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-800">
                        ✓ Layanan terdaftar di Master Data:
                        <a href="{{ route('admin.splp.services.show', $item->splp_service_id) }}" class="font-semibold underline">{{ $item->service->kode_layanan ?? '#'.$item->splp_service_id }}</a>
                    </div>
                @endif
            </div>
        ...
        {{-- Panel Manajemen Status --}}
        <div class="space-y-6">
            <div class="bg-white rounded-lg shadow-sm border p-6">
                <h3 class="font-bold text-gray-800 mb-4">Manajemen Status</h3>
                <form action="{{ route('admin.splp.provider.status', $item->id) }}" method="POST" class="space-y-4"
                    ...
                    <div>
                        ...
                        <select name="status" x-model="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            @foreach (\App\Models\SplpProviderRequest::statusLabels() as $val => $label)
                                @continue($val === 'draft')
                    ...
                    <div class="border rounded-lg p-3 bg-gray-50 space-y-2">
                        ...
                        @foreach (['check_administrasi' => 'Administrasi lengkap', 'check_teknis' => 'Verifikasi teknis (TLS, backend, autentikasi)', 'check_dokumentasi' => 'Dokumentasi API sesuai', 'check_klasifikasi_data' => 'Klasifikasi data ditinjau'] as $field => $label)
                    ...
                    <p class="text-xs text-gray-500">Saat status <strong>Selesai</strong>, layanan otomatis dicatat ke Master Data SPLP.</p>
                </form>
            </div>
