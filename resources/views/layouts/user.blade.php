...
<body class="bg-green-50 text-gray-800 min-h-screen flex" x-data="{ sidebarOpen: true }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <!-- Judul Sidebar -->
        <div class="p-6">
            <a href="{{ route('user.dashboard') }}" class="text-2xl font-bold text-green-300 block">E-Layanan</a>
        </div>
       ...
       {{ request()->routeIs('user.dashboard') ? 'bg-green-700 text-white font-semibold' : '' }}">
                Dashboard
            </a>

            <!-- Menu Induk Permohonan -->
            @php $isPermohonanOpen = request()->routeIs('user.permohonan') || request()->is('permohonan/unggah') || request()->is('digital*'); @endphp
            <div x-data="{ open: {{ $isPermohonanOpen ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    class="w-full text-left py-2.5 px-4 rounded transition duration-200 hover:bg-green-700 hover:text-white">
                    Permohonan
                </button>

                <div x-show="open" class="ml-4 space-y-1" x-cloak>
                    {{-- Unggah Surat: tampil hanya untuk user terverifikasi (opsional) --}}
                    @if (auth()->user()?->is_verified)
                        <a href="{{ route('user.permohonan') }}"
                            class="block py-2 px-4 rounded transition duration-200 hover:bg-green-700 hover:text-white
                   {{ request()->routeIs('user.permohonan') ? 'bg-green-700 text-white font-semibold' : '' }}">
                            Unggah Surat
                        </a>
                        <a href="{{ route('user.digital') }}"
                            class="block py-2 px-4 rounded transition duration-200 hover:bg-green-700 hover:text-white
                   {{ request()->routeIs('user.digital') || request()->is('digital/*') ? 'bg-green-700 text-white font-semibold' : '' }}">
                            Formulir Digital
                        </a>
                    @else
                        {{-- Jika belum diverifikasi, tampilkan item non-klik + tooltip --}}
                        <span class="block py-2 px-4 rounded text-gray-300 cursor-not-allowed"
                            title="Akun anda belum diverifikasi oleh Administrator">
                            Unggah Surat
                        </span>
                        <span class="block py-2 px-4 rounded text-gray-300 cursor-not-allowed"
                            title="Akun anda belum diverifikasi oleh Administrator">
            ...
            <a href="{{ route('profile.edit') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-700 hover:text-white
       {{ request()->routeIs('profile.edit') ? 'bg-green-700 text-white font-semibold' : '' }}">
                Profile Pengguna
            </a>
        </nav>
    </aside>
    ...
    <div :class="sidebarOpen ? 'md:ml-64' : 'md:ml-0'" class="flex-1 flex flex-col transition-all duration-300">
        ...
        <!-- Navbar -->
        <header class="bg-white shadow-md border-b p-4 flex justify-between items-center">
            <div class="flex items-center gap-4">
                <button @click="sidebarOpen = !sidebarOpen"
                    class="text-2xl text-green-700 focus:outline-none transition-transform duration-300 transform"
        ...
        <main class="flex-1 p-6 bg-white shadow-inner">
            @if (session('unverified_block'))
                <div class="mb-4 rounded border border-yellow-300 bg-yellow-50 p-3 text-sm">
                    {{ session('unverified_block') }}
                </div>
            @endif
            ...
            @auth
                @if (!auth()->user()->is_verified)
                    <div class="mb-4 rounded border border-red-300 bg-red-50 p-3 text-sm">
                        Akun anda belum diverifikasi oleh Administrator. Silahkan hubungi pengelola layanan DKISP.
                    </div>
                @endif
            @endauth
            @yield('content')
