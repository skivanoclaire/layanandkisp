...
<body class="bg-gray-50 text-gray-800 min-h-screen flex" x-data="{ sidebarOpen: true }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        @php
            // Flags rute
            $atDashboard = request()->routeIs('admin.dashboard');
            $atPermohonan = request()->routeIs('admin.permohonan');
            $atEmail = request()->routeIs('admin.email.*');
            $atDigitalSection = $atEmail || request()->is('admin/digital/*'); // grup formulir digital
            $openPermohonan = $atPermohonan || $atDigitalSection; // buka accordion jika di dalamnya

            $atMonitorWeb = request()->routeIs('admin.monitorweb*');
            $atTikAssets = request()->routeIs('admin.tik.assets.*') || request()->routeIs('admin.tik.categories.*');
            $atTikBorrow = request()->routeIs('admin.tik.borrow.*');
            $atUsers = request()->routeIs('admin.users*');
        @endphp
        ...
        <nav class="mt-6 space-y-2">
            ...
            <a href="{{ route('admin.dashboard') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                  {{ $atDashboard ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                Dashboard
            </a>

            {{-- ACCORDION: Permohonan --}}
            <div x-data="{ open: {{ $openPermohonan ? 'true' : 'false' }} }">
                {{-- Tombol parent: TIDAK highlight saat anak aktif; hanya hover --}}
                <button @click="open = !open"
                    class="w-full text-left py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600">
                    Permohonan
                </button>

                <div x-show="open" class="ml-4 space-y-1" x-cloak>
                    {{-- Leaf: Manual - Unggah Surat --}}
                    <a href="{{ route('admin.permohonan') }}"
                        class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                          {{ $atPermohonan ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                        Manual - Unggah Surat
                    </a>

                    {{-- SUB-ACCORDION: Formulir Digital --}}
                    @php
                        $openDigital = $atDigitalSection; // buka jika di dalam salah satu anak
                    @endphp
                    <div x-data="{ open: {{ $openDigital ? 'true' : 'false' }} }" class="space-y-1">
                        {{-- Header sub: JANGAN highlight saat anak aktif; hanya hover --}}
                        <button @click="open = !open"
                            class="w-full text-left py-2.5 px-4 rounded transition duration-200
                        ...
                        <div x-show="open" class="ml-4 space-y-1" x-cloak>
                            ...
                            <a href="{{ route('admin.email.index') }}"
                                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                                  {{ $atEmail ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                                Email
                            </a>

                            {{-- Tambah leaf lain di sini
                        <a href="{{ route('admin.xxx.index') }}"
                           class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                                  {{ request()->routeIs('admin.xxx.*') ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                            Nama Layanan
                        </a> --}}
                        </div>
                    </div>
            ...
            {{-- Web Monitor (leaf) --}}
            <a href="{{ route('admin.monitorweb') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                  {{ $atMonitorWeb ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                Web Monitor
            </a>
            ...
            <a href="{{ route('admin.tik.assets.index') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                  {{ $atTikAssets ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                Inventaris Digital
            </a>

            {{-- Tracing Peminjaman (leaf) --}}
            <a href="{{ route('admin.tik.borrow.index') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                  {{ $atTikBorrow ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                Laporan Peminjaman
            </a>
          ...
          {{ request()->routeIs('op.tik.schedule.*') ? 'bg-blue-100 text-blue-700' : '' }}">
            ...
            <a href="{{ route('op.tik.statistic.index') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-blue-100 hover:text-blue-700
          {{ request()->routeIs('op.tik.statistic.*') ? 'bg-blue-100 text-blue-700' : '' }}">
                Statistik Video Konferensi
            </a>

            {{-- User Management (leaf) --}}
            <a href="{{ route('admin.users') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
                  {{ $atUsers ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                User Management
            </a>

            {{-- Tambahkan di aside admin --}}
            <a href="{{ route('admin.simpeg.index') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-600
   {{ request()->routeIs('admin.simpeg.*') ? 'bg-green-100 text-green-600 font-semibold' : '' }}">
                Cek via SIMPEG
            </a>
