{{-- resources/views/layouts/operator-vidcon.blade.php --}}
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <title>Operator Vidcon @yield('title')</title>
    <!-- Favicon -->
...
<body class="bg-gray-50 text-gray-800 min-h-screen flex" x-data="{ sidebarOpen: true }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="p-6">
            <a href="{{ route('op.tik.borrow.index') }}" class="text-2xl font-bold text-green-300"
                x-show="sidebarOpen">E-Layanan</a>
            <a href="{{ route('op.tik.borrow.index') }}" class="text-2xl font-bold text-green-300"
                x-show="!sidebarOpen">EL</a>
        </div>
        ...
        <nav class="mt-6 space-y-2">
            ...
            <a href="{{ route('op.tik.borrow.index') }}"
               ...
               {{ request()->routeIs('op.tik.borrow.index') || request()->routeIs('op.tik.borrow.show') ? 'bg-green-100 text-green-700' : '' }}">
                Peminjaman Saya
            </a>
            ...
            <a href="{{ route('op.tik.borrow.create') }}"
                class="block py-2.5 px-4 rounded transition duration-200 hover:bg-green-100 hover:text-green-700
               {{ request()->routeIs('op.tik.borrow.create') ? 'bg-green-100 text-green-700' : '' }}">
                Buat Peminjaman
            </a>
            {{-- Jadwal Vidcon (BARU) --}}
        ...
        <header class="bg-green-200 shadow-md border-b p-4 flex justify-between items-center">
            <div class="flex items-center gap-4">
                <button @click="sidebarOpen = !sidebarOpen"
                    ...
                    :class="sidebarOpen ? '' : 'rotate-180'">☰</button>
                <img src="/logokaltarafix.png" alt="Logo" class="w-8 h-10">
                <span class="text-lg font-semibold text-green-700">Dashboard Operator Vidcon</span>
            </div>
            <div class="flex items-center gap-4">
                <span class="hidden md:block text-gray-700">Halo, {{ Auth::user()->name }}</span>
